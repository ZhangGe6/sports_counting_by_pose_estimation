...
class Camera2BasicFragment : Fragment(), FragmentCompat.OnRequestPermissionsResultCallback {
  ...
  private var sportsgroup: RadioGroup? = null
  private var radiogroup: RadioGroup? = null

  private var yangwoCount : Int = 0
  private var tiqianCount : Int = 0
  private var gaotaiCount : Int = 0

  private var yangwo_thre_x : Float = 600.0F   //vertical  \rightarrow
  private var gaotai_thre_y : Float = 900.0F   //horizontal \downarrow

  private var jointList = ArrayList<PointF>()
  //private var jointList_p = ArrayList<PointF>()
  private var head = PointF(0F,0F); private var head_p = PointF(0F,0F)
  private var l_knee = PointF(0F,0F); private var l_knee_p = PointF(0F,0F)
  private var r_knee = PointF(0F,0F); private var r_knee_p = PointF(0F,0F)
...
*/
  private var yangwo_lockFlag: Boolean = false
  private var gaotai_lockFlag: Boolean = false

  private var valid_jitter: Boolean = false
  private var tmp_p: Float = 0.0F    // for avoiding invalid jitter


  private var sportsKind: String = "yangwo"
  /**
  ...
  private val periodicClassify = object : Runnable {
    override fun run() {
      synchronized(lock) {
        if (runClassifier) {
          jointList = classifyFrame()
          /*
            0-head; 1-neck;
            2-l_shoulder; 3-l_elbow; 4-l_wrist; 8-l_hip; 9-l_knee; 10-l_ankle
            5-r_shoulder; 6-r_elbow; 7-r_wrist; 11-r_hip; 12-r_knee; 13-r_ankle
          */
          if (sportsKind=="yangwo"){
            ...
            valid_jitter = abs(head2hip_vertical-tmp_p)<15
            if (!yangwo_lockFlag && head2hip_vertical > hip2ankle){
              yangwo_lockFlag = true
              yangwoCount += 1
            }
            if (abs(head2hip_vertical) < 100 && valid_jitter && neck2hip_vertical != 0.0F) {
              yangwo_lockFlag = false
            }
            //showToast(yangwo_lockFlag.toString())
            //showToast(head2hip_vertical.toString())
            showToast("当前仰卧起坐计数: "+ yangwoCount)
            tmp_p = head2hip_vertical
          }

          else if (sportsKind=="gaotai"){
            var neck2hip: Float = jointList[8].y - jointList[1].y
            var neck2knee: Float = jointList[12].y - jointList[1].y    // to left knee
            //showToast(neck2hip.toString()+" "+neck2lknee.toString())
            if (!gaotai_lockFlag && neck2knee < 1.2*neck2hip){
              gaotaiCount += 1
              gaotai_lockFlag = true
            }
            if (neck2knee > 1.3*neck2hip){
              gaotai_lockFlag = false
            }
            //showToast(gaotai_lockFlag.toString())
            showToast("当前高抬腿计数: "+ gaotaiCount)
          }

          else if (sportsKind=="tiqian"){
            showToast("当前体前屈计数: "+ tiqianCount)
          }
  ...
  override fun onViewCreated(
    ...
    sportsgroup = view.findViewById(R.id.sportsgroup);
    ...
    sportsgroup!!.setOnCheckedChangeListener { group, checkedId ->
      if(checkedId==R.id.yangwo){
        sportsKind = "yangwo"
      } else if(checkedId==R.id.tiqian) {
        sportsKind = "tiqian"
      } else if(checkedId==R.id.gaotai){
        sportsKind = "gaotai"
      }
    }
  }
   ...
   */
  private fun classifyFrame():ArrayList<PointF> {
  //private fun classifyFrame():PointF {
    if (classifier == null || activity == null || cameraDevice == null) {
      showToast("Uninitialized Classifier or invalid context.")
      jointList.add(PointF(0F, 0F))

      //return jointList
    }

    // 进行关键点的提取（保存在classifier对象中）
    val bitmap = textureView!!.getBitmap(classifier!!.imageSizeX, classifier!!.imageSizeY)
    var textToShow = classifier!!.classifyFrame(bitmap)
    bitmap.recycle()

    // 将关键点传给drawView绘图，顺便将对应信息传出
    jointList = drawView!!.setDrawPoint(classifier!!.mPrintPointArray!!, 0.5f)
    //head = drawView!!.setDrawPoint(classifier!!.mPrintPointArray!!, 0.5f)
    // 输出运动计数+绘图刷新
